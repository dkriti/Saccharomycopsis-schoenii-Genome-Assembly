...
GTF_FILE = "schoenii_annotated.gtf"
FASTA_FILE = "Schoenii_assembly.fa"
OUTPUT_DATA_FILE = "intron_lengths.txt"
OUTPUT_PLOT_FILE = "Intron_Distribution.png"

# Regulatory genes and cytoskeleton factors
CULPRIT_KEYWORDS = [
    "RPL", "RPS",       # Ribosomal Proteins (Massive group)
    ...
    "HAC1",             # Unfolded Protein Response (Check manually if not found!)
    "GLC7"              # Phosphatase
]

def parse_fasta(fasta_path):
    """Reads genome fasta into a dictionary for consensus checking."""
    genome = {}
    current_chrom = None
    seq_parts = []
    try:
        ...
        print("NOTE: FASTA file not found. Skipping consensus sequence checks.")
        return None
...
def analyze_gtf():
    ...
    if intron_lengths:
        with open(OUTPUT_DATA_FILE, "w") as f:
            for l in intron_lengths:
                f.write(f"{l}\n")
        print(f"\n[SUCCESS] Exported {len(intron_lengths)} intron lengths to '{OUTPUT_DATA_FILE}'")
        ...
        try:
            ...
            print(f"[SUCCESS] Generated histogram plot: '{OUTPUT_PLOT_FILE}'")
    ...
    print("\n Usual Culprits (Conserved Intron-Containing Genes)")
    culprits_found.sort()
